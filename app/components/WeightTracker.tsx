...
export const WeightTracker: React.FC = () => {
    ...
    return (
        <div className="bg-white border border-zinc-100 rounded-3xl p-4 shadow-sm flex flex-col shrink-0 min-h-[120px] justify-center transition-all">
            ...
            {!showInput ? (
                <div className="flex flex-col items-center justify-center py-1 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <p className="text-[9px] font-bold text-zinc-300 uppercase tracking-widest mt-0.5">
                        Last recorded
                    </p>
                </div>
            ) : (
                    ...
                    >
                        {isLoading ? <Loader2 size={24} className="animate-spin" /> :
                            isSaved ? <Check size={24} className="text-green-400" /> :
                                <span className="font-bold text-sm uppercase">Save</span>}
                    </button>
                </div>
            )}
        </div>
