...
export const WorkoutDetails: React.FC<WorkoutDetailsProps> = ({ workout, onClose }) => {
    ...
    const handleSave = async () => {
        ...
        if (success) {
            setIsEditing(false);
            onClose(); // Optional: close or just stay
        }
    };
        ...
        setEditedWorkout({
            ...
            exercises: editedWorkout.exercises.map(ex => {
                if (ex.exerciseId === exerciseId) {
                    return {
                        ...ex,
                        sets: [...ex.sets, { setId: Math.random().toString(36).substr(2, 9), weight: "", reps: "" }]
                    };
                }
                return ex;
            })
            ...
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        {!isEditing ? (
                            workout.workoutTime && (
                                <p className="text-zinc-400 text-xs font-bold uppercase tracking-widest mt-1">
                                    Started at {workout.workoutTime}
                                </p>
                            )
                        ) : (
                            <div className="mt-2">
                                <input
                                    type="time"
                                    value={editedWorkout.workoutTime || ""}
                                    onChange={(e) => setEditedWorkout({ ...editedWorkout, workoutTime: e.target.value })}
                                    className="bg-zinc-800 text-white text-xs font-bold p-1 rounded border-none focus:ring-1 ring-zinc-500"
                                />
                            </div>
                        )}
                    </div>
                            ...
                            <div key={exercise.exerciseId || idx}>
                                ...
                                <div className="space-y-2">
                                    {exercise.sets.map((set, sIdx) => (
                                        <div
                                            key={set.setId || sIdx}
                                            className="grid grid-cols-12 gap-4 p-3 bg-zinc-50 rounded-xl items-center border border-transparent hover:border-zinc-200 transition-colors"
                                        >
                                            ...
                                            <div className="col-span-4 flex justify-center">
                                                {!isEditing ? (
                                                    ...
                                                    <input
                                                        type="number"
                                                        value={set.weight}
                                                        onChange={(e) => updateSet(exercise.exerciseId, set.setId, "weight", e.target.value)}
                                                        className="w-16 bg-white border border-zinc-200 rounded p-1 text-center text-sm font-bold focus:ring-1 ring-black"
                                                    />
                                                )}
                                            </div>
                                            ...
                                            <div className="col-span-4 flex justify-center">
                                                {!isEditing ? (
                                                    ...
                                                    <input
                                                        type="number"
                                                        value={set.reps}
                                                        onChange={(e) => updateSet(exercise.exerciseId, set.setId, "reps", e.target.value)}
                                                        className="w-16 bg-white border border-zinc-200 rounded p-1 text-center text-sm font-bold focus:ring-1 ring-black"
                                                    />
                                                )}
                                            </div>
                                            ...
                                            <div className="col-span-2 flex justify-end">
                                                {isEditing && (isEditing ? editedWorkout : workout).exercises.find(ex => ex.exerciseId === exercise.exerciseId)?.sets.length! > 1 && (
                                                    <button
                                                        onClick={() => removeSet(exercise.exerciseId, set.setId)}
                ...
                <div className="p-6 bg-zinc-50 border-t border-zinc-100 flex justify-end gap-3">
                    {!isEditing ? (
                        <button
                            onClick={onClose}
                            className="px-6 py-2.5 bg-black text-white rounded-xl text-sm font-bold hover:bg-zinc-800 transition-all shadow-md"
                        >
                            Close
                        </button>
                    ) : (
                        <>
                            <button
                                ...
                                className="px-6 py-2.5 bg-white text-zinc-500 border border-zinc-200 rounded-xl text-sm font-bold hover:bg-zinc-50 transition-all"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="px-6 py-2.5 bg-black text-white rounded-xl text-sm font-bold hover:bg-zinc-800 transition-all shadow-md flex items-center gap-2 disabled:opacity-50"
                            >
